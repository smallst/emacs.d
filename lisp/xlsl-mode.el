                                        ;-*- coding: utf-8 -*-
;; xlsl-mode_Xah_Lee.el -- Major mode for editing XLSL (Linden Scripting Language).

;; Copyright © 2008 by Xah Lee

;; Author: Xah Lee ( http://xahlee.org/ )
;; Keywords: XLSL, Second Life, Linden Scripting Language
...
;; A major mode for editing XLSL (Linden Scripting Language).
...
;; You are done.
;; Open any XLSL source code, then type Alt+x xlsl-mode, you'll have syntax coloring and other features.
...
;; version 1.5.7, 2009-04-26 • Modified xlsl-color-vectors-region so that if a vector component is not a number between 0 to 1, no color is applied. Added chars such as < > + - * & etc to syntax table, so that emacs knows better when xlsl keyword char stops.
...
;; version 1.5, 2009-04-01 • Added a customizable variable xlsl-mode-format-style. Its value should be a integer. If 0, then no automatic formating are done. The Tab key insert a tab. If 1, then the formating style emulates that of embedded XLSL client (implemented by loading c-mode). If 2, then the formatting style is whatever you have setup for c-mode.
;; version 1.4.0, 2009-03-19. • Added a user function xlsl-color-vectors-region. This will color vector text such as “<1, 0, .5>” using its own value. • Much improved xlsl-convert-rgb. Now it converts text under cursor between #rrggbb and <r, g, b> formats. • Minor fix on xlsl-complete-symbol, so that it wont give lisp error when there's no text before cursor. • Minor implementation cleanup on xlsl-lookup-xlsl-ref and xlsl-lookup-xlsl-ref2.
...
;; version 1.3.2.1, 2009-03-04. Fix on xlsl-lookup-xlsl-ref and xlsl-lookup-xlsl-ref2 so that if a region contain a space char, it still works.
;; version 1.3.2, 2009-03-04. Added a second xlsl wiki ref lookup: xlsl-lookup-xlsl-ref2. One for looking up “xlslwiki.net”, and the other for “wiki.secondlife.com”.
...
;; version 1.2.3, 2009-03-02. Changed the file name from “xlsl-mode_Xah_Lee.el” to “xlsl-mode.el”.
...
(defcustom xlsl-xlslint-path ""
  "Full path to xlslint utility.
The value must be xlslint's full path, not its parent dir.
This variable is used by the command `xlsl-syntax-check'.
xlslint can be downloaded at http://w-hat.com/xlslint as of 2009-03."
  :type '(file :must-match t)
  :group 'xlsl-mode)

(defcustom xlsl-mode-format-style 1
  "Specifies how source code auto formatting is done.
The value should be a integer. If 0, then no automatic formating are done. The Tab key insert a tab. If 1, then the formating style emulates that of embedded XLSL client (implemented by loading c-mode). If 2, then the formatting style is whatever you have setup for c-mode."
  :type '(integer)
  :group 'xlsl-mode)

(defcustom xlsl-reference-url "http://xlslwiki.net/xlslwiki/wakka.php?wakka="
  "URL for XLSL reference website.
The value is used by `xlsl-lookup-xlsl-ref'.
The value should be one of:
“http://xlslwiki.net/xlslwiki/wakka.php?wakka=”
...
(defcustom xlsl-reference-url2 "http://wiki.secondlife.com/wiki/"
  "URL for XLSL reference website.
The value is used by `xlsl-lookup-xlsl-ref2'.
...
(if xlsl-mode-map nil
  ...
  (define-key xlsl-mode-map (kbd "C-c C-r") 'xlsl-lookup-xlsl-ref)
  (define-key xlsl-mode-map (kbd "C-c C-f") 'xlsl-lookup-xlsl-ref2)
  ...
  (let ((menuMap (make-sparse-keymap "XLSL")))
    (define-key xlsl-mode-map [menu-bar xlsl] (cons "XLSL" menuMap))
...
http://xahlee.org/sl/ls-emacs.html \n\n")
    ...
    (define-key menuMap [lookup-onlne-doc2] '("Lookup ref second site" . xlsl-lookup-xlsl-ref2))
    (define-key menuMap [lookup-onlne-doc] '("Lookup ref on current word" . xlsl-lookup-xlsl-ref))
...
(defun xlsl-syntax-check ()
  "Validate XLSL syntax of the current file.
You need to have xlslint installed, and have `xlsl-xlslint-path' set."
  (interactive)
  (if (not (= (length xlsl-xlslint-path) 0))
      (shell-command (concat xlsl-xlslint-path " \"" (buffer-file-name) "\""))
    (message "Error: xlsl-xlslint-path not set.")))
...
(defun xlsl-lookup-xlsl-site (site-url)
  "Switch to browser to particular url.
SITE-URL is a url string.
This is a internal function.
This function is called by xlsl-lookup-xlsl-ref and xlsl-lookup-xlsl-ref2."
...
(defun xlsl-lookup-xlsl-ref ()
  "Look up current word in XLSL ref site in a browser.
...
See also `xlsl-lookup-xlsl-ref2'."
  (interactive)
  (xlsl-lookup-xlsl-site xlsl-reference-url))

(defun xlsl-lookup-xlsl-ref2 ()
  "Look up current word in XLSL ref site in a browser.
If there is a text selection (a phrase), lookup that phrase.
Set variable xlsl-reference-url2 if you want to change the url used.

See also `xlsl-lookup-xlsl-ref'."
  (interactive)
  (xlsl-lookup-xlsl-site xlsl-reference-url2))
...
;; keyword completion
(defvar xlsl-kwdList nil "XLSL keywords.")
...
Keywords include all XLSL's event handlers, functions, and CONSTANTS."
  (interactive)
  (let ((posEnd (point))
...
(defun company-xlsl-mode-backend (command &optional arg &rest ignored)
  (interactive (list 'interactive))

  (case command
    (interactive (company-begin-backend 'company-xlsl-mode-backend))
...
(define-derived-mode xlsl-mode fundamental-mode
  ;; (define-derived-mode xlsl-mode c-mode "XLSL"
  "Major mode for editing XLSL (Linden Scripting Language).
Shortcuts             Command Name
\\[comment-dwim]       `comment-dwim'
\\[xlsl-complete-symbol]      `xlsl-complete-symbol'
\\[xlsl-lookup-xlsl-ref]     `xlsl-lookup-xlsl-ref'
\\[xlsl-lookup-xlsl-ref2]    `xlsl-lookup-xlsl-ref2'
